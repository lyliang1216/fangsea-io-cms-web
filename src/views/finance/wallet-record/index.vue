    ...
    <div class="app-container">
        ...
        <el-form :inline="true" ref="filter" :model="filter" class="filter-form-inline">
            ...
            <el-form-item label="用途" prop="usageType">
                <el-select v-model="filter.usageType" clearable filterable size="small" placeholder="请选择">
                    <el-option
                        v-for="item in usageType_type"
                        v-if="item.code!==3"
                        :key="item.code"
                        :label="item.name"
                        :value="item.code">
        ...
        <el-table
            ...
            <el-table-column
                ...
                label="用途">
                <template slot-scope="props">
                    {{getusageType(props.row.usageType)}}
                </template>
            </el-table-column>
            <el-table-column
                prop="comments"
            ...
            <el-table-column
                ...
                width="100">
                <template slot-scope="props">
                    {{props.row.state | getstate(props.row.stateReason)}}
                </template>
            </el-table-column>
        </el-table>
...
<script>
    import {getrecordlist, getcurrencylist, getusetypelist} from '@/api/finance/wallet-record'
    ...
    export default {
        ...
        methods: {
            ...
            getusageTypetype () {
                ...
                getusetypelist(params).then(res => {
                    if (res.code.toString() === '0') {
                        this.usageType_type = res.data
                    }
                }).catch(() => {
                })
            },
             ...
             */
            getusageType (val) {
                for (let i in this.usageType_type) {
                    if (val && this.usageType_type[i].code !== null) {
                        if (this.usageType_type[i].code.toString() === val.toString()) {
                            return this.usageType_type[i].name
                        }
                    }
                }
            },
